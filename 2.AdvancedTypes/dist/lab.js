...
function findLowestPrices(input) {
    ...
    input.forEach(function (entry) {
        ...
        if (!(product in productPrices) || price < productPrices[product].price) {
            productPrices[product] = { town: town, price: price };
        }
    });
    for (var productName in productPrices) {
        var productInfo = productPrices[productName];
        console.log("".concat(productName, " -> ").concat(productInfo.price, " (").concat(productInfo.town, ")"));
    }
}
// Example usage:
var input = [
