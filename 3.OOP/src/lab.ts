...
class Trainer {
  ...
  processCommand(command: string): void {
    const element = command.trim();
    const hasElement = this.pokemons.some(
...
function processTournament(input: string[]): void {
  const trainers: { [key: string]: Trainer } = {};

  let i = 0;
  while (input[i] !== 'Tournament') {
    const [trainerName, pokemonName, element, healthStr] = input[i].split(' ');
    const health = parseInt(healthStr);
  ...
  const sortedTrainers = Object.keys(trainers)
    .map((trainerName) => trainers[trainerName])
    .sort((a, b) => {
